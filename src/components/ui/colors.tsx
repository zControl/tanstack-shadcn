...
const ColorBox = ({ label, bgColor, textColor }: colorBoxProps) => {
  return (
    <div
      className={`h-12 px-4 flex items-center justify-center bg-${bgColor} text-${textColor} text-lg font-semibold`}
    >
      {label}
    </div>
  );
...
const Colors = () => {
  return (
    <div className="px-8 grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <ColorBox label="Accent" bgColor="accent" textColor="accent-foreground" />
      <ColorBox
        label="Destructive"
        bgColor="destructive"
      ...
      />
      <ColorBox label="Header" bgColor="header" textColor="header-foreground" />
      <ColorBox label="Border" bgColor="border" textColor="border-foreground" />
